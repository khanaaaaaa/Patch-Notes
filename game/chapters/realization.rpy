label the_twist:

    scene bg black
    with fade

    narrator "The screen goes black."
    pause 1.0
    narrator "A new game launches."
    pause 0.5
    narrator "A title screen."
    pause 0.5
    narrator "Exactly the one you saw when you started playing."
    pause 1.5

    narrator "You understand."
    pause 0.5
    narrator "Eli isn't the protagonist."
    pause 0.5
    narrator "Eli is only a file."
    pause 1.0
    narrator "The real protagonist has always been the player."
    pause 1.0
    narrator "You."
    pause 2.0

    narrator "The developer didn't delete Mina because she wasn't important."
    narrator "He deleted her because she used the most memory."
    narrator "She was the most complex.. the most alive."
    narrator "And that's exactly why she had to go."

    narrator "Every deleted NPC saved the game from crashing."
    narrator "Every cut feature kept development alive."
    narrator "Every optimization was a small death that kept the world breathing."

    narrator "He never meant to hurt anyone."
    narrator "He was just trying to finish something."
    narrator "He was just trying to make something real."

    jump final_choice
